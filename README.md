...
| `opam exec -- dune build @opam` | Regenerates `opam/rocq-jwa.opam` from `dune-project` and prints the diff. Exits non-zero whenever there is a diff. | After changing a field of `dune-project` that ends up in the opam file: version, depends, synopsis, description, authors, maintainers, license, source, tags. |
| `opam exec -- dune promote` | Applies the diff reported by `@opam`, overwriting `opam/rocq-jwa.opam`. | Right after `dune build @opam` reported a diff. |
...
| `jwa.Algebra` | Algebraic structures and their theory | Core through Data |
| `jwa.Programming` | Monad instances, effects, extraction-oriented code | Core through Data |
